...
class graph:
    ...
    def add_edge(self, u : int, v : int) -> None:
        ...
        try:
            if u not in self.get_edge_label.keys():
                self.get_edge_label[u] = {v : self.edge_num}
            else:
                self.get_edge_label[u].update({v : self.edge_num})
            if v not in self.get_edge_label.keys():
                self.get_edge_label[v] = {u : self.edge_num}
            else:
                self.get_edge_label[v].update({u : self.edge_num})
        except:
            pass
...
class face(graph):
    ...
    def change_boundary(self, old_boundary, new_boundary) -> None:
        if len(new_boundary) > 2:
            for v in new_boundary[1 : -1]:
                self.V[v] = vertex(v)
        for i in range(len(new_boundary) - 1):
            self.add_edge(new_boundary[i], new_boundary[i + 1])
    ...
    def print(self) -> None:
        print("FACE : " + str(self.face_label))
        start = int(0)
        for key, i in self.V.items():
            start = key
            break
        self.init_dfs()
        self.V[start].visited = True
        ptr = self.V[start].neighbors[0]
        print(start, end = '')
        while ptr != start:
            self.V[ptr].visited = True
            print(" -> " + str(ptr), end = '')
            if not self.V[self.V[ptr].neighbors[0]].visited:
                ptr = self.V[ptr].neighbors[0]
            elif not self.V[self.V[ptr].neighbors[1]].visited:
                ptr = self.V[ptr].neighbors[1]
            else:
                break
        print()
...
class H_seg(graph):
    ...
    def add_edge(self, u, v, Label):
        ...
        self.V[v].neighbors.append(u)
        if u not in self.get_edge_label.keys():
            self.get_edge_label[u] = {v : Label}
        else:
            self.get_edge_label[u].update({v : Label})
        if v not in self.get_edge_label.keys():
            self.get_edge_label[v] = {u : Label}
        else:
            self.get_edge_label[v].update({u : Label})
        self.E[Label] = edge(Label = Label, u = u, v = v)

    def find_hseg(self, Current, G : graph, H):
        G.V[Current].visited = True
        if Current not in self.V.keys():
            self.V[Current] = vertex(Current)
        for v in G.V[Current].neighbors:
            ...
            if v not in H["vertexs"] and not G.V[v].visited:
                self.find_hseg(v, G, H)
            if v in H["vertexs"] and v not in self.connect_point:
                self.connect_point.append(v)
...
def DMP(G  : graph) -> bool:
    ...
    for i in range(len(first_circle)-1):
        H["edges"] += [G.get_edge_label[first_circle[i]][first_circle[i+1]]]
...
def DFSBLK(G : graph, u : int):
    ...
    for v in G.V[u].neighbors:
        if not G.V[v].visited:
            S.append(G.get_edge_label[u][v])
            parent[v] = u
            if u in children.keys():
                children[u] += 1
            else:
                children[u] = 1
            DFSBLK(G, v)
            low[u] = min(low[u], low[v])
            if (u not in parent.keys() and children[u] >= 2) or (u in parent.keys() and low[v] >= d[u]):
                block = [S.pop(-1)]
                while block[-1] != G.get_edge_label[u][v]:
                    block.append(S.pop(-1))
                blocks.append(block)
        elif u not in parent.keys() or v != parent[u]:
            if d[u] > d[v]:
                S.append(G.get_edge_label[u][v])
            low[u] = min(low[u], d[v])
...
if __name__ == "__main__":
    ...
    for E in blocks:
        v = {}
        for e in E:
            if G.E[e].U in v.keys():
                v[G.E[e].U] += 1
            else:
                v[G.E[e].U] = 1
            if G.E[e].V in v.keys():
                v[G.E[e].V] += 1
            else:
                v[G.E[e].V] = 1
        num_of_subG = int(0)
        vertex_of_subG = []
        for v_label in v.keys():
            if v[v_label] > 1:
                num_of_subG += 1
                vertex_of_subG.append(v_label)
        subG = graph(num_of_subG)
        for e in E:
            if G.E[e].U in vertex_of_subG and G.E[e].V in vertex_of_subG:
                subG.add_edge(vertex_of_subG.index(G.E[e].U) + 1, vertex_of_subG.index(G.E[e].V) + 1)
        sub_Gs.append(subG)
    ans = True
    for g in sub_Gs:
        if len(g.V) > 0:
